...
const count = (target, input) => {
  ...
  return input.reduce((total, currentArray) => {
    return total + currentArray.filter(number => number === target).length;
  }, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((total, currentArray) => {
    return total + currentArray.reduce((sum, number) => sum + number, 0);
  }, 0);
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(({ gender }) => gender === 'male' || gender === 'female')
...
let findShortest = (data) => {
  ...
  return data.reduce((shortest, character) => {
    return (shortest === null || parseInt(character.height) < parseInt(shortest.height)) ? character : shortest;
  }, null).name;
};
